...
while($i -lt $len){
    ...
    elseif ($infosLigne0[0] -match "union|Rattrapages"){
    ...
    $increment = 2 # Par défaut, on passe au cours suivant (qui est 2 lignes plus bas)
    ...
    if (($i + 2) -lt $len) {
        ...
        if ($premierMot -cmatch "^[\p{Lu}\-]+$"){
            ...
            $increment = 3 # On a trouvé un prof, donc le prochain cours est 3 lignes plus bas
        }
    }
...
$coursArray | ConvertTo-Json -Compress -Depth 3 | Out-File -FilePath ".\cours.json" -Encoding UTF8
